...
SoftwareSerial rfSerial(rxPin, txPin); // RX, TX

const int myFeederID = 1; // Unique ID for this feeder

void setup() {
  Serial.begin(9600);
  rfSerial.begin(9600); // Initialize SoftwareSerial port
}

void loop() {
  // Check if a command is received via RF
  if (rfSerial.available()) {
    String receivedMessage = rfSerial.readStringUntil('\n');
...
void sendCompletionSignal() {
  ...
  rfSerial.print(message); // Send completion signal over RF
